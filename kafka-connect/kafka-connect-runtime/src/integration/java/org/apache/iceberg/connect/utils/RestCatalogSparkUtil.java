...
public class RestCatalogSparkUtil {
    ...
    public static String runSparkSqlQuery(String query) throws InterruptedException {
        var exec = DOCKER_CLIENT.execCreateCmd(getContainer(SPARK_IMAGE).getId())
                ...
                .withCmd(
                        ...
                        "--conf", format("spark.sql.catalog.spark_catalog.uri=http://iceberg:%s", CATALOG_PORT),
                        "--conf", format("spark.sql.catalog.spark_catalog.s3.endpoint=http://minio:%s", MINIO_PORT),
                        "--conf", format("spark.sql.catalog.spark_catalog.s3.access-key-id=%s", AWS_ACCESS_KEY),
                        "--conf", format("spark.sql.catalog.spark_catalog.s3.secret-access-key=%s", AWS_SECRET_KEY),
                        "--conf", "spark.sql.catalog.spark_catalog.s3.path-style-access=true",
                        "--conf", format("spark.sql.catalog.spark_catalog.s3.region=%s", AWS_REGION),
        ...
        DOCKER_CLIENT.execStartCmd(exec.getId())
