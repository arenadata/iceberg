...
public class TestDeletionVectors extends IntegrationTestBaseV3 {
    ...
    public void testDeletionVector(boolean useSchema, String writeDeleteMode, boolean isMergeOnReadEnabled)
            throws InterruptedException {
        Map<String, Object> connectorConfigs =
                Map.of("iceberg.tables.auto-create-props.write.delete.mode", writeDeleteMode);

        runTest(useSchema, addConnectorConfigs(V3_AUTO_CREATE_CONNECTOR_CONFIGS, connectorConfigs), List.of(TABLE_IDENTIFIER), KAFKA_BASE_EVENTS);
        ...
        runSparkSqlQuery(format("DELETE FROM %s.%s WHERE id=2", ICEBERG_REST_CATALOG, TABLE_IDENTIFIER));

        String tableDataPath = format("%s/data/", loadCatalogTableLocation(loadCatalogTable(catalog(), TABLE_IDENTIFIER)))
                .replaceFirst("/", "");
        ...
        assertThat(S3_CLIENT.listObjectsV2(ListObjectsV2Request.builder()
                        .bucket("bucket")
                        .prefix(tableDataPath)
                        .build())
                .contents()
                .stream()
                .anyMatch(o -> o.key().endsWith(".puffin"))).isEqualTo(isMergeOnReadEnabled);
