...
public class TestVariantShredding extends CatalogTestBase {
  ...
  @TestTemplate
  public void testNestedVariantUsesExistingWriterPath() throws IOException {
    validationCatalog.dropTable(tableIdent, true);
    Schema nestedSchema =
        new Schema(
            Types.NestedField.required(1, "id", Types.IntegerType.get()),
            Types.NestedField.optional(
                ...
                "payload",
                Types.StructType.of(
                    Types.NestedField.optional(3, "address", Types.VariantType.get()))));
    validationCatalog.createTable(
        tableIdent, nestedSchema, null, Map.of(TableProperties.FORMAT_VERSION, "3"));
    spark.conf().set(SparkSQLProperties.SHRED_VARIANTS, "true");

    sql(
        "INSERT INTO %s VALUES " + "(1, named_struct('address', parse_json('{\"city\":\"NYC\"}')))",
        tableName);
    ...
    try (CloseableIterable<FileScanTask> tasks = table.newScan().planFiles()) {
      ...
      for (FileScanTask task : tasks) {
        ...
        try (ParquetFileReader reader = ParquetFileReader.open(inputFile)) {
          GroupType payload = reader.getFileMetaData().getSchema().getType("payload").asGroupType();
          GroupType address = payload.getType("address").asGroupType();
          assertThat(address.containsField("typed_value")).isFalse();
          assertThat(address.containsField("value")).isTrue();
        }
      }
    }

    List<Object[]> rows =
        sql("SELECT variant_get(payload.address, '$.city', 'string') FROM %s", tableName);
    assertThat(rows).hasSize(1);
    assertThat(rows.get(0)[0]).isEqualTo("NYC");
  }
  ...
  public void testNestedObjectType() throws IOException {
    ...
    String values =
        "(1, parse_json('{\"location\": {\"city\": \"Seattle\", \"zip\": 98101}, \"tags\": "
            + "[\"java\", \"scala\", \"python\"]}')), "
  ...
  @TestTemplate
  public void testReadMixedUnshreddedAndShreddedFiles() throws IOException {
    spark.conf().set(SparkSQLProperties.SHRED_VARIANTS, "false");
    sql("INSERT INTO %s VALUES (1, parse_json('{\"kind\":\"plain\"}'))", tableName);

    spark.conf().set(SparkSQLProperties.SHRED_VARIANTS, "true");
    sql("INSERT INTO %s VALUES (2, parse_json('{\"kind\":\"shredded\"}'))", tableName);

    int shreddedFiles = 0;
    int unshreddedFiles = 0;
    Table table = validationCatalog.loadTable(tableIdent);
    try (CloseableIterable<FileScanTask> tasks = table.newScan().planFiles()) {
      for (FileScanTask task : tasks) {
        HadoopInputFile inputFile =
            HadoopInputFile.fromPath(new Path(task.file().location()), new Configuration());
        try (ParquetFileReader reader = ParquetFileReader.open(inputFile)) {
          GroupType address = reader.getFileMetaData().getSchema().getType("address").asGroupType();
          if (address.containsField("typed_value")) {
            shreddedFiles += 1;
          } else {
            unshreddedFiles += 1;
          }
    ...
    assertThat(shreddedFiles).isEqualTo(1);
    assertThat(unshreddedFiles).isEqualTo(1);

    List<Object[]> rows =
        sql("SELECT id, variant_get(address, '$.kind', 'string') FROM %s ORDER BY id", tableName);
    assertThat(rows).hasSize(2);
    assertThat(rows.get(0)[1]).isEqualTo("plain");
    assertThat(rows.get(1)[1]).isEqualTo("shredded");
  }

  @TestTemplate
  public void testPartitionedMultiWriterRoundTrip() throws IOException {
    validationCatalog.dropTable(tableIdent, true);
    PartitionSpec spec = PartitionSpec.builderFor(SCHEMA).identity("id").build();
    validationCatalog.createTable(
        tableIdent, SCHEMA, spec, Map.of(TableProperties.FORMAT_VERSION, "3"));
    spark.conf().set(SparkSQLProperties.SHRED_VARIANTS, "true");
    ...
    GroupType name =
        field(
            ...
            shreddedPrimitive(
                PrimitiveType.PrimitiveTypeName.BINARY, LogicalTypeAnnotation.stringType()));
    verifyParquetSchema(
        validationCatalog.loadTable(tableIdent),
        parquetSchema(variant("address", 2, Type.Repetition.REQUIRED, objectFields(name))));
